...
qsql := 'select studyId,
                         case when studyId = ''TAS3681_101'' then concat(''TAS3681101_'',siteId)
                         else concat(studyId,concat(''_'',siteId)) end as siteid,
                         usubjId,
                         queryId,
                         formId,
                         visit,
            ...
            SELECT left("study"::text, strpos("study", '' - '') - 1)::text AS studyId,
                        left("sitename"::text, strpos("sitename", ''-'') - 1)::text AS siteId,
...
"log"::int AS log_num,
        ...
        WHERE lower(left("markinggroupname", 9)) = ''site from''
           OR lower(right(trim("markinggroupname"), 7)) = ''to site'')a';
        ELSE
